...
if feed_mode == "竞价成功：80%机制电价 + 20%现货":
    st.success(f"**✅ 稳健型结算 (竞价成功)**：当前模型严格适用**增量光伏项目上网电量的80%享受机制电价（{mech_price}元）**[cite: 2]。在湖南省 4 月至 6 月现货市场均价低迷（低至 0.075-0.15 元）的大环境下，该方案利用 80% 的机制电价作为“压舱石”，能够大幅对冲汛期负电价或低谷击穿成本线的财务风险。")
else:
    st.warning(f"**⚠️ 激进型敞口 (全额现货)**：当前模型适用**未参与竞价的全现货结算**[cite: 2]。参照湖南 2026 年二季度现货结算水平（约 {spot_mean} 元），汛期余电上网收益面临严重缩水。建议通过储能错峰放电、或尽快获取指标参与竞价获取机制电价保障。")
